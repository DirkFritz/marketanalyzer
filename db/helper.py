...
def companies_data(db):
    companies = pd.DataFrame(
        db.select("companies"),
        columns=["Symbol", "Name", "Shares", "Sector", "Industry", "Category"],
    )
    return companies
...
def historic_stock_data_date(db, date, symbols):
    data_db = db.select_date(date, "historic", symbols)
    data_db_df = pd.DataFrame(
        columns=["Symbol", "DateTime", "Open", "Close", "High", "Low", "Volume"]
    )
    data_db_df = pd.concat(
